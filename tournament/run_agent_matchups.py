#!/usr/bin/env python3
"""
MicroRTS Matchup Runner

Supports:
1. Default tournament mode:
   - all valid submissions vs reference anchors
   - optional submission head-to-head

2. Focus mode:
   - pick ONE agent and run it against:
       a) all other submissions
       b) built-in anchor AIs
       c) both
   - useful when you want to benchmark one agent only

Example commands:

Default tournament:
    python3 tournament/run_agent_matchups.py --games 2

Default tournament without submission-vs-submission head-to-head:
    python3 tournament/run_agent_matchups.py --games 2 --skip-h2h

Focus one submission against all other submissions + built-in AIs:
    python3 tournament/run_agent_matchups.py --focus-agent YeBot --focus-targets all --games 2

Focus one submission against built-in AIs only:
    python3 tournament/run_agent_matchups.py --focus-agent YeBot --focus-targets anchors --games 3

Focus one built-in AI against all submissions:
    python3 tournament/run_agent_matchups.py --focus-agent ai.abstraction.WorkerRush --focus-targets submissions --games 2

Focus one agent against only specific opponents:
    python3 tournament/run_agent_matchups.py --focus-agent YeBot --opponents WorkerRush CoacAI another_submission_name --games 3
"""
...
def run_game(ai1, ai2, map_info, ai1_name="", ai2_name=""):
    """Run a single game and return result from AI1's perspective."""
    ...
    print(f"  {display1} vs {display2} on {map_info['label']}...", end=" ", flush=True)
...
def install_submission(submission_dir):
    ...
    Returns (fully_qualified_class, display_name, metadata).
    """
...
def opponent_breakdown(reference_games, max_cycles):
    """Build per-opponent stats dict for anchors."""
    breakdown = {}
...
def discover_and_install_submissions(submissions_dir):
    """Discover, validate, install submissions. Return contestants dict."""
...
def build_anchor_contestants():
    """Return anchors as contestant-like dict entries."""
    anchors = {}
    for fqcn, info in ANCHORS.items():
        anchors[fqcn] = {
...
def build_name_index(contestants, anchors):
    """
    Build a flexible lookup index using:
    - fqcn
    - display_name
    - team_name
    - short class name
    ...
    def add_key(key, fqcn):
        key = normalize_name(key)
        if key and key not in idx:
            idx[key] = fqcn

    merged = {}
    merged.update(contestants)
    merged.update(anchors)

    for fqcn, info in merged.items():
        add_key(fqcn, fqcn)
        add_key(info["display_name"], fqcn)
        add_key(info.get("team_name", ""), fqcn)
        add_key(fqcn.split(".")[-1], fqcn)

    return idx


def resolve_agent_identifier(agent_str, contestants, anchors):
    """Resolve a user-provided agent identifier to a fully qualified class name."""
    idx = build_name_index(contestants, anchors)
    key = normalize_name(agent_str)
    return idx.get(key)


def summarize_games(games, max_cycles):
    wins = sum(1 for g in games if g["result"] == "win")
    draws = sum(1 for g in games if g["result"] == "draw")
    losses = sum(1 for g in games if g["result"] not in ("win", "draw"))
    avg_ticks = round(sum(g["ticks"] for g in games) / len(games), 1) if games else 0
    avg_score = round(
        sum(calculate_game_score(g["result"], g["ticks"], max_cycles) for g in games) / len(games),
        3
    ) if games else 0.0
    return {
        "wins": wins,
...
def choose_focus_opponents(focus_fqcn, contestants, anchors, focus_targets, specific_opponents):
    """
    Build opponent pool for focus mode.
    focus_targets: anchors | submissions | all
    """
    pool = {}

    if focus_targets in ("submissions", "all"):
        for fqcn, info in contestants.items():
            if fqcn != focus_fqcn:
                pool[fqcn] = info

    if focus_targets in ("anchors", "all"):
        for fqcn, info in anchors.items():
            if fqcn != focus_fqcn:
                pool[fqcn] = info

    if specific_opponents:
        filtered = {}
        for opp in specific_opponents:
            resolved = resolve_agent_identifier(opp, contestants, anchors)
            if not resolved:
                print(f"[WARN] Could not resolve opponent: {opp}")
                continue
            if resolved == focus_fqcn:
                continue
            if resolved in pool:
                filtered[resolved] = pool[resolved]
            else:
                merged = {}
                merged.update(contestants)
                merged.update(anchors)
                if resolved in merged:
                    filtered[resolved] = merged[resolved]
        pool = filtered

    return pool


def run_focus_mode(focus_fqcn, focus_info, opponent_pool, games_per_pair, maps, results_dir):
    """
    Run one selected agent against all chosen opponents across all maps.
    No elimination; just exhaustive comparisons.
    """
    print("=" * 60)
    print("FOCUS MODE")
    print("=" * 60)
    ...
    print(f"Focus agent: {focus_info['display_name']} ({focus_fqcn})")
    print(f"Opponent count: {len(opponent_pool)}")
    print(f"Maps: {', '.join(m['label'] for m in maps)}")
    print(f"Games per matchup: {games_per_pair}")
    print()

    if not opponent_pool:
        print("No opponents selected.")
        return None

    results = {
        "focus_agent": {
            "fqcn": focus_fqcn,
            "display_name": focus_info["display_name"],
            "team_name": focus_info.get("team_name", focus_info["display_name"]),
            "kind": focus_info.get("kind", "unknown")
        },
        "date": now.isoformat(),
        "maps": maps,
    ...
    for opp_fqcn, opp_info in opponent_pool.items():
        opp_name = opp_info["display_name"]
        print(f"\n{focus_info['display_name']} vs {opp_name}")
        print("-" * 40)

        results["opponents"][opp_fqcn] = {
            "display_name": opp_name,
            "team_name": opp_info.get("team_name", opp_name),
            "kind": opp_info.get("kind", "unknown"),
            "maps": {}
        }
        ...
        for map_info in maps:
            ...
            for game_num in range(games_per_pair):
                result = run_game(
                    ...
                    opp_fqcn,
                    map_info,
                    focus_info["display_name"],
                    opp_name
                )
                result["game_num"] = game_num + 1
                games.append(result)

            results["opponents"][opp_fqcn]["maps"][map_label] = {
                "map": map_info["path"],
                "max_cycles": map_info["max_cycles"],
                "games": games,
                "summary": summarize_games(games, map_info["max_cycles"])
            }

    print("\n" + "=" * 60)
    print("FOCUS MODE SUMMARY")
    print("=" * 60)
    header = f"{'Opponent':<25}{'Map':<8}{'W':<4}{'D':<4}{'L':<4}{'AvgTicks':<10}{'AvgScore':<10}"
    print(header)
    print("-" * len(header))

    for opp_fqcn, opp_data in results["opponents"].items():
        for map_label, map_data in opp_data["maps"].items():
            s = map_data["summary"]
            print(
                f"{opp_data['display_name']:<25}"
                f"{map_label:<8}"
                f"{s['wins']:<4}"
                f"{s['draws']:<4}"
                f"{s['losses']:<4}"
                f"{s['avg_ticks']:<10}"
                f"{s['avg_game_score']:<10}"
            )

    Path(results_dir).mkdir(exist_ok=True)
    results_file = f"{results_dir}/focus_{normalize_name(focus_info['display_name']).replace(' ', '_')}_{timestamp}.json"
...
def run_default_tournament(games_per_pair=1, skip_h2h=False, submissions_dir="submissions"):
    """Run the original full tournament behavior."""
    print("=" * 60)
    ...
    contestants = discover_and_install_submissions(submissions_dir)
    if not contestants:
        sys.exit(1)
...
def parse_args():
    parser = argparse.ArgumentParser(description="MicroRTS matchup and tournament runner")

    parser.add_argument("--games", type=int, default=1, help="Games per matchup")
    parser.add_argument("--skip-h2h", action="store_true", help="Skip submission-vs-submission head-to-head in default tournament mode")
    parser.add_argument("--submissions-dir", default="submissions", help="Directory containing submissions")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Run only one selected agent against chosen opponents. Accepts display_name, team_name, short class name, or full class name."
    )
    parser.add_argument(
        "--focus-targets",
        choices=["anchors", "submissions", "all"],
        default="all",
        help="Opponent pool for focus mode"
    )
    parser.add_argument(
        "--opponents",
        nargs="*",
        default=None,
        help="Optional list of specific opponents for focus mode"
    )
...
def main():
    ...
    contestants = discover_and_install_submissions(args.submissions_dir)
    anchors = build_anchor_contestants()
    ...
    if args.focus_agent:
        focus_fqcn = resolve_agent_identifier(args.focus_agent, contestants, anchors)
        if not focus_fqcn:
            print(f"Could not resolve focus agent: {args.focus_agent}")
            print("Available submissions:")
            for fqcn, info in contestants.items():
                print(f"  {info['display_name']} ({fqcn})")
            print("Available built-in anchors:")
            for fqcn, info in anchors.items():
                print(f"  {info['display_name']} ({fqcn})")
            sys.exit(1)
        ...
        merged.update(contestants)
        merged.update(anchors)
        focus_info = merged[focus_fqcn]

        opponent_pool = choose_focus_opponents(
            focus_fqcn=focus_fqcn,
            contestants=contestants,
            anchors=anchors,
            focus_targets=args.focus_targets,
            specific_opponents=args.opponents
        )
    ...
    run_default_tournament(
        games_per_pair=args.games,
        skip_h2h=args.skip_h2h,
